...
def build_label_or_cluster_vectors(labels, base_dir, rare_threshold, label_space):
    ...
    Returns (mlb, label_vectors, rare_labels, not_rare_labels).
    """
    ...
    logging.info(f"[{label_space}] Rare labels (freq < {rare_threshold}): {len(rare_labels)}")
    logging.info(f"[{label_space}] Not rare labels (freq >= {rare_threshold}): {len(not_rare_labels)}")
    logging.info(f"[{label_space}] Label partitions and classes saved to {partition_file}")

    return mlb, label_vectors, rare_labels, not_rare_labels
...
def preprocess_data(df, tokenizer, base_dir, max_length=512, rare_threshold=50, label_space="label", candidate_budget=256):
    ...
    mlb, label_vectors, rare_labels, not_rare_labels = build_label_or_cluster_vectors(
...
def main(
    ...
    logging.info("📊 Using k values of 5, 8, 15, and 25 for evaluation metrics...")
    trainer = train_model(
        custom_model,
        training_args,
        train_dataset,
