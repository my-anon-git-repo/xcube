...
# -------- MASK IDENTITY IN settings.ini --------
if [ -f settings.ini ]; then
  echo
  echo "🎭 Masking identity fields in settings.ini..."
  sed -i \
    -e "s#^doc_host = .*#doc_host = https://my-anon-git-repo.github.io#" \
    -e "s#^git_url = .*#git_url = https://github.com/my-anon-git-repo/xcube#" \
    -e "s#^author = .*#author = Anonymous#" \
    -e "s#^author_email = .*#author_email = anonymous@users.noreply.github.com#" \
    -e "s#^copyright = .*#copyright = 2023 onwards, Anonymous#" \
    -e "s#^user = .*#user = my-anon-git-repo#" \
    settings.ini
fi
