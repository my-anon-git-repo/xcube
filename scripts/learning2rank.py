...
def preprocess_data(
    ...
    def verify_token_mappings(
            ...
            for token_idx in token_sample_indices:
                try:
                    ...
                    if 0 <= token_id < tokenizer.vocab_size:
                        ...
                        print(
                            f"Token ID {token_id} out of range (valid range: 0-{tokenizer.vocab_size-1})"
                        )
        ...
        if mismatches:
            ...
            if tokenizer.vocab_size != len(df_toks):
                logging.error(
                    f"❌ Tokenizer's vocab size = {tokenizer.vocab_size}, while df_tok's size = {len(df_toks)}"
                )
            raise ValueError(
    ...
    def process_batch(examples):
        ...
        for text, is_valid, labels in zip(
            ...
            chunks = chunk_text(text, is_valid, labels, tokenizer, max_len, overlap=10)
...
def define_model(base_model, num_labels, lookup_table):
    ...
    config = LTRConfig(num_labels=num_labels,  ground_model_name_or_path=ground_model_name_or_path ,**config_dict)
    ...
    def evaluate(self, eval_dataset=None, ignore_keys=None, metric_key_prefix="eval"):
        ...
        with torch.no_grad():
            for batch in evaluation_bar:
                ...
                batch_metrics = self.compute_metrics((batch_logits, batch_labels))
...
def save_and_push(
    ...
    try:
        ...
        # Verify critical files/directories before pushing
        required_files = ["model.safetensors", "config.json", "lookup_table.pt", "ground_model"]
        missing_files = []
...
def load_tokenizer(task_hub_model_id):
    ...
    try:
        ...
        tokenizer = AutoTokenizer.from_pretrained(task_hub_model_id)
        tokenizer.padding_side = "right"
        if tokenizer.pad_token is None:
            logging.info("📝 Setting pad token to eos token...")
            tokenizer.pad_token = tokenizer.eos_token
...
def main(
    ...
    # df_data = pd.concat([
    #     df_data[df_data['is_valid']].sample(n=2, random_state=random_seed),
    #     df_data[~df_data['is_valid']].sample(n=2, random_state=random_seed)
    # ])
    frac = 0.001 * 0.3
    df_data = pd.concat(
        [
            df_data[df_data["is_valid"]].sample(frac=frac, random_state=random_seed),
            df_data[~df_data["is_valid"]].sample(frac=frac, random_state=random_seed),
        ]
    )
    # quick iteration <remove later>
    ...
    if do_fresh_training or not (model_exists_locally or model_exists_on_hub):
        try:
            ...
            if load_from_checkpoint:
                ...
                # Define custom model
                custom_model = define_model(base_model, num_labels, lookup_table)
    ...
    custom_model, train_dataset, eval_dataset, training_args, accelerator = (
        prepare_training_components(
            custom_model, dataset, output_dir, task_hub_model_id, hf_token
        )
    )
